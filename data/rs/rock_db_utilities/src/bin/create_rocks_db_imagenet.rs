...
fn create_hashmap(data: &[[&str; 3]]) -> HashMap<String, usize> {
    let mut map = HashMap::new();
    for &[dir_name, class_id, _] in data {
        let class_id: usize = class_id.parse().unwrap();
        map.insert(dir_name.to_string(), class_id - 1);
    }
    map
}

fn count_jpeg_files<P: AsRef<Path>>(path: P) -> usize {
    let mut count = 0;
    for entry in fs::read_dir(path).unwrap() {
        let entry = entry.unwrap();
        let path = entry.path();
        if path.is_dir() {
            count += count_jpeg_files(&path);
        } else if path.extension().and_then(|s| s.to_str()) == Some("JPEG") {
            count += 1;
        }
    }
    count
...
fn main() {
    ...
    let class_to_idx = create_hashmap(&data);
    let start = Instant::now();
    println!("Start: {:?}", start);
    let args: Vec<String> = env::args().collect();
    if args.len() != 3 {
        println!("Usage: {} <ImageNet directory> <RocksDB directory>", args[0]);
        return;
    }

    let imagenet_dir = &args[1];
    let rocksdb_dir = &args[2];

    let mut options = Options::default();
    options.create_if_missing(true);
    options.set_compression_type(DBCompressionType::Zlib); // Use maximum compression ratio
    options.optimize_for_point_lookup(512); // Optimize for storage
    let db = DB::open(&options, rocksdb_dir).unwrap();

    let total_files = count_jpeg_files(imagenet_dir);    
    println!("JPEG files counted: {:?}", Instant::now());
    // Create a new progress bar
    let pb = ProgressBar::new(total_files as u64);
    pb.set_style(ProgressStyle::default_bar()
    .template("{spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} ({eta})")
    .unwrap() // Unwrap the Result value
    .progress_chars("#>-"));

    let mut id: usize = 0;
    for entry in fs::read_dir(imagenet_dir).unwrap() {
        let entry = entry.unwrap();
        let path = entry.path();
        if path.is_dir() {
            let dir_name = path.file_name().unwrap().to_str().unwrap().to_string(); // Clone the path here
            for file in fs::read_dir(&path).unwrap() {
                let file = file.unwrap();
                let file_path = file.path();
                if file_path.extension().unwrap().to_str().unwrap() == "JPEG" {
                    let class_id = class_to_idx[&dir_name];
                    let mut value = vec![];
                    value.extend(&class_id.to_le_bytes());
                    value.extend(fs::read(file_path).unwrap());
                    db.put(&id.to_le_bytes(), &value).unwrap();
                    id += 1;
                    pb.inc(1);
                }
            }
    ...
    db.put(max_id_key, &id.to_le_bytes()).unwrap();
    println!("Compacting database: {:?}", Instant::now());
    db.compact_range(None::<&[u8]>, None::<&[u8]>); // Compact the entire database
