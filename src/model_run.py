...
if os.environ.get('RWKV_CUDA_ON') == '1':
    ...
    try:
        load(
            name=f"wkv_cuda",
            sources=[f"{parent_path}/cuda/wrapper.cpp", f"{parent_path}/cuda/operators.cu", f"{parent_path}/cuda/gemm_fp16_cublas.cpp"],
            verbose=True,
            extra_ldflags=["cublas.lib" if os.name == "nt" else ""],
            extra_cuda_cflags=["--use_fast_math", "-O3", "--extra-device-vectorization"],
            is_python_module=False)
        print("cuBLAS matmul built successfully.")
        ...
        load(
            name=f"wkv_cuda",
            sources=[f"{parent_path}/cuda/wrapper.cpp", f"{parent_path}/cuda/operators.cu"],
            verbose=True,
            extra_cuda_cflags=["--use_fast_math", "-O3", "--extra-device-vectorization"],
