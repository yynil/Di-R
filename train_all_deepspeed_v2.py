    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if trainer.is_global_zero:  # logging   
            ...
            if real_step % args.log_every_n_steps == 0:
                ...
                ema_file = os.path.join(output_dir, 'ema.pt')
                print(f'saving ema to {ema_file}')
                torch.save(args.ema.state_dict(), ema_file)
...
def main(args):
    ...
    dataset = ZipFastDataset(args.data_path,transforms=transform,tokenizer=tokenizer)
    max_len = 77 
    eos_id = tokenizer.eot_token_id
    pad_id = 0
    def collate_fn(batch):
        ...
        new_ids = [x + [pad_id] * (max_len - len(x)) for x in new_ids]
    ...
    requires_grad(model.text_encoder, False)  # Text encoder should not be updated by gradients
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default='/media/yueyulin/TOUROS/images/laion400mZip')
    parser.add_argument("--results-dir", type=str, default="/media/yueyulin/KINGSTON/tmp")
